...
class TestTS3Connection(TestCase):
    ...
    def test_parse_resp_kicked_event(self):
        resp = (
            b"notifyclientleftview cfid=1 ctid=0 reasonid="
            + str(ReasonID.SERVER_KICK.value).encode("ascii")
            + b" reasonmsg=Kicked. clid=1"
        )
        result = self.conn._parse_resp(resp)
        self.assertIs(
            ClientKickedEvent, type(result), "Client kick not parsed correctly"
        )
    ...
    def test_parse_banned_event(self):
        resp = (
            b"notifyclientleftview cfid=1 ctid=0 reasonid="
            + str(ReasonID.BAN.value).encode("ascii")
            + b" reasonmsg=Kicked. clid=1 bantime=10 invokerid=2 invokername=Test invokeruid=sdfsadf"
        )
        result = self.conn._parse_resp(resp)
        self.assertIs(
            ClientBannedEvent, type(result), "Client ban not parsed correctly"
        )
        ...
        self.assertEqual(
            "sdfsadf", result.invoker_uid, "Client ban not parsed correctly"
        )
    ...
    def test_parse_resp_left_event_missing_reason_id(self):
        ...
        self.assertEqual(
            1, result.client_id, "ClientLeft without reason id not parsed correctly"
        )
    ...
    def test_parse_resp_left_event_empty(self):
        ...
        result = self.conn._parse_resp(resp)
        self.assertIs(
            ClientLeftEvent, type(result), "Empty client left not parsed correctly"
        )
